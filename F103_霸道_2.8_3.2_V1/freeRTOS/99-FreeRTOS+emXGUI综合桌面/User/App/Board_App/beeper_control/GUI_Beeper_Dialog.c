...
static void press_button_OwnerDraw(DRAWITEM_HDR *ds)
{
  ...
  BitBlt(hdc, 29, 40, 59, 101, hdc_beeper_png[hdc_beeper_horn], 0, 0, SRCCOPY);
  
  switch (press_flag)
  {
    case 3:
      BitBlt(hdc, 114, 33, 29, 116, hdc_beeper_png[hdc_beeper_horn3], 0, 0, SRCCOPY);
    
    case 2:
      BitBlt(hdc, 105, 51, 21, 79, hdc_beeper_png[hdc_beeper_horn2], 0, 0, SRCCOPY);

    case 1:
      BitBlt(hdc, 96, 71, 13, 40, hdc_beeper_png[hdc_beeper_horn1], 0, 0, SRCCOPY);
...
//      res = FS_Load_Content(GUI_BEEPER_BACKGROUNG_PIC, (char**)&jpeg_buf, &jpeg_size);
      ...
      for (uint8_t xC=0; xC<hdc_beeper_end; xC++)
      {
        ...
        ClrDisplay(hdc_beeper_png[beeper_png_info[xC].id], NULL, 0);
          res = RES_Load_Content(beeper_png_info[xC].pic_name, (char**)&pic_buf, &pic_size);
...
//          res = RES_Load_Content(clock_png_info[xC].pic_name, (char**)&pic_buf, &pic_size);
    ...
    case WM_PAINT:
    {
      ...
      RECT rc1 = {50, 0, GUI_XSIZE-100, 26};
      ...
      if (Load_Flag)     // 资源加载完成
      {
        ...
        DrawText(hdc, L"蜂鸣器控制", -1, &rc1, DT_VCENTER|DT_CENTER);//绘制文字(居中对齐方式)
      }
      else
      {    /* 资源加载未完成 */ 
    ...
    case WM_DESTROY:
    {
      ...
      for (uint8_t xC=0; xC<hdc_beeper_end; xC++)
      {
        DeleteDC(hdc_beeper_png[beeper_png_info[xC].id]);    // 删除 DC
      }
